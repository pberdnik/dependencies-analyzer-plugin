...
public class MyDependenciesUsagesPanel extends UsagesPanel {
  private final List<MyDependenciesBuilder> myBuilders;

  public MyDependenciesUsagesPanel(Project project, final List<MyDependenciesBuilder> builders) {
    super(project);
    myBuilders = builders;
        ...
        ApplicationManager.getApplication().runReadAction(() -> {
          ...
          try {
            if (myBuilders.get(0).isBackward()){
              ...
              usages = MyFindDependencyUtil.findBackwardDependencies(myBuilders, searchFor, searchIn);
            }
            else {
              elementsToSearch = searchFor;
              usages = MyFindDependencyUtil.findDependencies(myBuilders, searchIn, searchFor);
            }
  ...
  public void addBuilder(MyDependenciesBuilder builder) {
